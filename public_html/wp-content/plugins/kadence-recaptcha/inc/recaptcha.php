...
class Kadence_Recaptcha_Fontend {
	...
	public function recaptcha_field( $before = '', $after = '', $id = null, $styles = '', $action = 'submit' ) {
		...
		if ( 2 == self::settings( 'enable_v3' ) ) {
			...
			$out .= '<span class="cf-turnstile" data-sitekey="' . esc_attr( self::settings( 'turnstile_site_key' ) ) . '" data-action="' . esc_attr( $action ) . '" data-theme="' . esc_attr( self::settings( 'kt_re_theme' ) ) . '">'; 
			$out .= '</span>';
			$out .= '</p>';
